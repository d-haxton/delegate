...
namespace Delegate.Tera.Common.game.services
{
    public class NpcDatabase
    {
        ...
        private List<NpcInfo> _npcInfo;

        public NpcDatabase(List<NpcInfo> npcInfo)
        {
            _npcInfo = npcInfo;
            _zoneNames = (from npc in npcInfo.GroupBy(x => x.HuntingZoneId).Select(x => x.FirstOrDefault())
        ...
        {
            return _npcInfo.FirstOrDefault(t => t.HuntingZoneId == huntingZoneId && t.TemplateId == templateId);
        }
